...
class ServeTimeCheckMixin:
    ...
    def set_bounds(self: SlotFormProtocol, field: str, attr: Literal["max"] | Literal["min"], value: str):
...
class DiningInfoForm(ConcurrenflictFormMixin, ServeTimeCheckMixin, forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.set_bounds(
            "sign_up_deadline",
            "min",
            (self.instance.date - settings.DINING_SLOT_CLAIM_AHEAD).strftime("%Y-%m-%dT00:00"),
        )
    ...
    def clean(self):
        ...
        if dinner_cost_total and dining_cost:
            ...
            if self.instance.diners.count() > 0:
                ...
                raise ValidationError({"dining_cost_total": ValidationError(msg, code="costs_no_diners")})
...
class DiningListDeleteForm(forms.ModelForm):
    ...
    def __init__(self, deleted_by, instance, data=None, **kwargs):
        ...
        self.entry_delete_forms = [
            DiningEntryDeleteForm(entry, deleted_by, {}) for entry in instance.dining_entries.all()
        ]
